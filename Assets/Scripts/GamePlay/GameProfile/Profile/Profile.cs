...
{	
		...
		protected void setBool (string tag, bool value)
		{
				...
				PlayerPrefs.SetString (DataEncryption.Encrypt (tag), DataEncryption.Encrypt (val.ToString ()));
		}
		...
		{
				PlayerPrefs.SetString (DataEncryption.Encrypt (tag), DataEncryption.Encrypt (value.ToString ()));
		}
		...
		protected float getFloat (string tag)
		{
				float result = 0;
				try {
						result = float.Parse (DataEncryption.Decrypt (PlayerPrefs.GetString (DataEncryption.Encrypt (tag))), CultureInfo.InvariantCulture);
				} catch {
						result = 0;
				}
